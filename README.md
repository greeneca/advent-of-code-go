...
The program will automatically download the input for the day when running a problem if it does not already exist.

## Running
To run a problem, use the following command:

```bash
go run aoc.go <year> <day>-<part> (input file)
```

`input file` is optional. If not provided, the program will use the default input from the advent of code website.

## Example

```bash
go run aoc.go 2017 1-1
```
This will run the first part of day 1 of 2017. The input will be downloaded if it does not already exist.

```bash
go run aoc.go 2017 1-2 1-1
```
